...
const YourStack = ({selectedTechnology,setSelectedTechnology}:IYourStackProps) => {
    const handleRemoveTechnology=((technology:ITechnologiesProps)=>{
        ...
        selectedTechnology=> selectedTechnology.name !== technology.name)
            console.log(restTechnology, "restTechnology")
            setSelectedTechnology(restTechnology)
    })
    if(selectedTechnology.length === 0){
        return <div className="ml-2 h-auto w-auto border-1 max-h-50 border-gray-200 rounded-[7px] m-1 p-2 shadow-gray-300 shadow-md container "> 
        <div className="text-2xl font-bold">
                Your Stack
            
            </div>
            <h2 className=" text-gray-500 mb-2 font-medium">No technologies selected yet</h2>
...
const handleRemoveAllTechnology =((technology:ITechnologiesProps)=>{
  console.log(technology, "remove all technology")
    ...
    return (
        ...
        <div>
            ...
            <div className="flex gap-3 items-center">
             <img src={technology.image} alt={technology.name} className="h-[30px] w-[30px]" />
             <div><h2>{technology.name}</h2>
             <h2 className="text-[10px]">{technology.category}</h2></div>
         </div>
        <span className="cursor-pointer" onClick={()=> handleRemoveTechnology(technology)}>
            <RxCross2 />
        </span>
      ...
      <button  onClick={()=> handleRemoveAllTechnology(technologies)}
     className="btn btn-active btn-secondary container mx-auto mt-5">Remove All</button>
